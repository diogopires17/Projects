...
def insert_user(email, name, password, password2):
    ...
    try:
        ...
        return False, "2There was an error validating your input, please check your data and try again."
...
def change_password():
    if request.method == 'POST':
        ...
        if password != confirm_password:
            flash("Passwords do not match. Please try again.", category='error') # corrigir
...
def add_review():
    if request.method == 'POST':
        ...
        # Insert the review into the database without any modification
...
def checkout():
    ...
    return render_template('checkout.html', cc_info=cc_info, error_message=error_message)
...
def move_to_orders():
    ...
    if save_cc_info:
        ...
        cursor.execute(f"INSERT INTO credit_card_info (user_id, name, cc_number, exp_month, exp_year, cvv) VALUES ({user_id}, '{name}', {cc_number}, {exp_month}, {exp_year}, {cvv})")
        connection.commit()
    else:
...
def update_cart_ADD(user_id, product_id):
    ...
    try:
        ...
        cursor.execute(f"INSERT INTO cart (user_id, product_id, quantity) VALUES ({user_id}, {product_id}, 1)")
        connection.commit()
        return False
...
def update_wishlist_ADD(user_id, product_id):
    try:
        ...
        cursor.execute(f"INSERT INTO wishlist (user_id, product_id, quantity) VALUES ({user_id}, {product_id}, 1)")
        connection.commit()
        return False
...
@app.route('/checout')
def provoke_database_error():
    try:
        connection = sqlite3.connect('data.db')
        cursor = connection.cursor()
        cursor.execute('SELECT * FROM checkout_table')
    except Exception as e:
        return render_template('checout.html', error_message=str(e)), 500  #intentionally provoke an error comiting typo on html page
...
def update_cart_REORDER(user_id, product_id, quantity):
    ...
    if existing_quantity:
        ...
        cursor.execute(f"INSERT INTO cart (user_id, product_id, quantity) VALUES ({user_id}, {product_id}, {quantity})")      
        connection.commit()
        return False
